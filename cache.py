...
class TwitterSearchApp:
    ...
    def search_cache(self, entity, keyword=None, hashtag=None, user_deets=[], lang="Select"):
        ...
        cache[keyword] = {
            "result": result if result is not None else [],  # Ensure result is not None
            "timestamp": time.time()
        }
...
#         return self.search_cache('hashtag', keyword)
    ...
    def tweets_for_users(self, user_id, input_keyword, input_hashtag, input_language):
        ...
        if input_hashtag:
            query_criteria["$or"] = [{"entities.hashtags.text": {"$in": input_hashtag, "$options": "i"}}]
        if input_language != "Select":
