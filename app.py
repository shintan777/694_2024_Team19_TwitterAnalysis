...
def search_page():
    ...
    if form_submit_button:
        if user_search:
            ...
            # Set query params based on input values
            params = {
                "page": "results",
...
def results_page(keyword=None, hashtag=None, language="Select", start_date=None, end_date=None): 
    ...
    input_hashtag = st.experimental_get_query_params().get("hashtag", "")
    ...
    if input_hashtag:
        if not isinstance(input_hashtag, list):
            input_hashtag = [input_hashtag]
        query["entities.hashtags.text"] = {"$in": input_hashtag}
    ...
    if original_tweets:
        ...
        top_tweets = []
        if input_keyword:
            top_tweets += app.search_cache(entity="tweet", keyword=input_keyword, hashtag=input_hashtag, lang=input_language)
        if input_hashtag:
            top_tweets += app.search_cache(entity="hashtag", keyword=input_keyword, hashtag=input_hashtag, lang=input_language)
